...
async function run() {
  ...
  /** 공식 제조사 로고: `client/public/manufacturer-logos/official-mfr-NN.png` 와 동일 순서 */
  const OFFICIAL_MANUFACTURER_SEED = [
  ...
  const allPartners = await Partner.insertMany([...OFFICIAL_MANUFACTURER_SEED, ...SYNTHESIS_SEED]);
  const manufacturer = allPartners[0];
  const manufacturer2 = allPartners[1];
  const manufacturer3 = allPartners[2];
  const synthesis = allPartners[OFFICIAL_MANUFACTURER_SEED.length];
  const synthesis2 = allPartners[OFFICIAL_MANUFACTURER_SEED.length + 1];
